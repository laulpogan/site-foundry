...
function deriveSubject(spec: SiteSpec): string {
  const assumption = spec.assumptions.find((item) => /\bsite for (?:an? )?(.+?) for\b/i.test(item));
  const match = assumption?.match(/\bsite for (?:an? )?(.+?) for\b/i);
  return titleCase(match?.[1]?.trim() || spec.primary_goal.replaceAll("-", " "));
}
